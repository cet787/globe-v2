...
func _ready() -> void:
	...
	if noise_value < 0.0:
		material_override = preload("res://Globe/water_material.tres").duplicate()
		cell_type = CellType.WATER
		available_resources = "None"
	elif noise_value >= 0.0 and noise_value < 0.35:
		material_override = preload("res://Globe/grass_material.tres").duplicate()
		cell_type = CellType.TREES
		#inactive_position *= 1.005
		#active_position *= 1.005
		available_resources = "Wood"
	elif noise_value >= 0.35:
		material_override = preload("res://Globe/dirt_material.tres").duplicate()
