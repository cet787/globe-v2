...
func _ready() -> void:
	...
	$Panel/VBoxContainer/MoveButton.pressed.connect(_on_move_ship_pressed)
...
func _process(delta):
	...
	if seek_cell:
		...
		if local_direction.length_squared() < 0.00001:
		...
		if abs(angle) < 0.05:
			...
			if nearest_cell.cell_type != Cell.CellType.WATER:
				message_panel.display_message("You cannot drive ship on land.")
				seek_cell = null
				return
...
func highlight(timeout: float = INF):
	$Cube.material_overlay.set_shader_parameter("highlight_strength", 1.0)
	if timeout != INF:
		await get_tree().create_timer(timeout).timeout
		$Cube.material_overlay.set_shader_parameter("highlight_strength", 0.0)
...
func select():
	...
	$Cube.material_overlay.set_shader_parameter("selected_highlight_strength", 1.0)
	$Panel/VBoxContainer/Label .text = """Battleship
			Ammo: {ammo}
...
func unselect():
	$Cube.material_overlay.set_shader_parameter("selected_highlight_strength", 0.0)
	$Panel.visible = false
...
func _on_move_ship_pressed():
	var camera: Camera = get_tree().current_scene.get_node("Camera")
	
	if camera.selected_object is BattleShip:
		var previous_selected_object: BattleShip = camera.selected_object
		...
		if camera.selected_object.cell_type != Cell.CellType.WATER:
			print("Battle ship can only move to water regions")
			message_panel.display_message("You can only move a battleship to water type cells")
			return
